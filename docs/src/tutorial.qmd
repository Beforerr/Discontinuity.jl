...
```{julia}
using Speasy
using Discontinuity: detect_variance, compute_indices
using Dates
...
```{julia}
@b @eval compute_indices(data, tau)
@profview_allocs compute_indices(data, tau)
@descend compute_indices(data, tau)
```
